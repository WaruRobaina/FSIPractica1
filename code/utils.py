...
class ramification(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda nodo: nodo.path_cost, reverse=True)

    def pop(self):
        return self.A.pop()

class ramification_understimated(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda nodo: nodo.path_cost + self.problem.h(nodo), reverse=True)
